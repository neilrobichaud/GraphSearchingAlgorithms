...
public class BinarySearch {
	/*
	 * will return index of closest latitude restaurant
	 */
    ...
    public static ArrayList<Restaurant> getlist(double restaurantlat, double restaurantlng, int closestlatindex){
    	...
    	i= closestlatindex;
    	while (Math.abs(FindPath.rlist[i].lat - restaurantlat) < 0.5 && FindPath.rlist[i].visited != true && i<FindPath.rlist.length){		//adds all unvisited restaurant withing +0.5 lat point
    		closeLatList.add(FindPath.rlist[i]);
    		i++;
    	}    	
